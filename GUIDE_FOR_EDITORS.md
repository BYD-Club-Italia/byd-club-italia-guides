...
1. Se sei stato invitato come collaboratore, accetta l’invito (arriva via email e compare un banner giallo sul repo)
1. Ora puoi proporre modifiche
...
1. Inserisci il nome (solo lettere, numeri e `_`, es. `link_telegram`) e il valore
1. Usa il pulsante **Variabile** della toolbar per inserirla nel testo
...
1. **Carica le immagini sulla stessa branch**
...
1. Estrai lo zip nella root del repo
1. Commit e push:
   ...
   ```
1. Il link per aprire la PR viene stampato direttamente nel terminale
...
1. Clicca l’icona **✏️ matita** in alto a destra
1. All’inizio del file trovi le variabili tra `---`:
   ...
   ---
   ```
1. Modifica il valore: `countrycode_target: "Singapore (65)"`
1. Clicca **Commit changes…**, seleziona *«Create a new branch and start a pull request»*, clicca **Propose changes**
1. Compila titolo PR (es. `Aggiorna CountryCode a Singapore`) e clicca **Create pull request**
1. Attendi l’approvazione. Puoi menzionare qualcuno con `@username` nei commenti
...
1. Cerca il testo (Ctrl+F nel browser), modifica come in un normale editor
1. Commit → PR → aspetta approvazione
...
1. **Add file → Upload files**, trascina l’immagine, fai commit su un branch nuovo
1. Nel file `.md` della stessa guida (sullo stesso branch), aggiungi:
...
1. Trova il commit da annullare
1. Clicca `< >` per vedere le modifiche
...
1. Tab **Files changed**: verde = aggiunto, rosso = rimosso
1. Se tutto va bene: **Review changes → Approve → Submit review**
1. Se c’è da correggere: **Request changes** con spiegazione nel commento

Una volta approvata, il merge viene eseguito dal maintainer del progetto ([@Tagliax10](https://t.me/Tagliax10)).
...
Controlla la tab [Actions](https://github.com/BYD-Club-Italia/byd-club-italia-guides/actions). Una ❌ rossa significa che il build è fallito. Clicca sull’errore per vedere il messaggio. Le cause più comuni sono:

- **Errore di sintassi** — es. `}}` mancante in una variabile `{{ }}`
- **Immagine mancante** — il file `.md` fa riferimento a un’immagine in `images/<slug>/` che non è stata caricata nel repo. Verifica che la cartella `images/<slug>/` esista e contenga tutti i file referenziati nella guida
