...
def preprocess_custom_syntax(md_text: str) -> str:
    ...
    """
    # Chapter heading color: "# Title {color=warning}" -> "# Title\n{color=warning}"
    # Keep inline — will be processed by a custom renderer below
    # (handled via attrs plugin which converts {key=val} into attributes)
    ...
    # Image width: ![alt](src){width=400} -> <img class="md-img" src="..." alt="..." style="max-width:400px">
    # We do this as HTML so we control the wrapper <figure>
...
def build_guide(guide_path: Path, template: jinja2.Template):
...
def build_index(guides: list, index_template: jinja2.Template):
    """Build landing page listing all guides."""
    output = index_template.render(guides=guides)
...
def main():
    ...
    for md_file in md_files:
        info = build_guide(md_file, guide_tmpl)
        built_guides.append(info)

    # Only rebuild index if we built all guides
    if not args.guide:
        build_index(built_guides, index_tmpl)
